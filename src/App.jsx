...
import { Plus, Wrench, Calendar, Info, MessageSquare, Send, Bot, User, Trash2, Cpu, ChevronRight, AlertCircle, Loader2, Coffee, ShieldCheck, ShieldAlert, Zap, List, Edit2, Link as LinkIcon, ExternalLink, X } from 'lucide-react';

// Wklej swój klucz API w to miejsce (pomiędzy cudzysłowy):
const apiKey = ""; 

export default function App() {
  // 1. Inicjalizacja stanu z pamięci LocalStorage
  ...
  // Zapisywanie zmian do pamięci na bieżąco
  useEffect(() => {
  ...
  const startEditDevice = (device) => {
    ...
    setActiveTab('add'); // formularz służy do dodawania i edycji
  };
  ...
  const deleteDevice = (id) => {
    ...
    // Uporządkowanie historii czatu po usuniętym urządzeniu
            ...
            <div className="p-4 sm:p-6 overflow-y-auto h-full relative">
              ...
              {devices.length === 0 ? (
                <div className="text-center py-12 text-slate-500 flex flex-col items-center">
                  ...
                  <p>Brak urządzeń w bazie.</p>
                  <button onClick={() => { resetForm(); setActiveTab('add'); }} className="mt-4 text-blue-600 hover:underline">
                    Dodaj swoje pierwsze urządzenie
                  </button>
                </div>
              ) : (
                      ...
                      <div className="flex-1">
                        ...
                        {device.manualUrl && (
                           <a href={device.manualUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[11px] sm:text-xs text-blue-700 bg-blue-50 hover:bg-blue-100 p-1.5 rounded-md w-fit border border-blue-200 mb-4 transition-colors">
                              <ExternalLink className="w-3 h-3" /> Instrukcja obsługi (PDF/Strona)
                           </a>
                        )}
                      </div>
...
function ChatInterface({ device, history, updateHistory }) {
  ...
  const callGeminiAPI = async (userText, retries = 5) => {
    if (!apiKey || apiKey.trim() === "") {
      return "🚨 **Błąd: Brak klucza API!** \n\nOtwórz plik `src/App.jsx` w Visual Studio Code i wklej swój klucz w linii 5:\n`const apiKey = \"TwójKlucz\";`";
    }
...
`;
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Kod ${response.status} - ${errorData.error?.message || 'Odrzucono przez serwer Google'}`);
        }
  ...
  return (
    <>
      {/* Header Czatu z opcją czyszczenia historii */}
      ...
      <div className="p-2 sm:p-4 bg-white border-t border-slate-200 flex flex-col shrink-0">
        <div className="flex gap-2 overflow-x-auto pb-2 mb-1 scrollbar-hide w-full">
